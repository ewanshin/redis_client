...
bool CTestBase::StartTest(const std::string &strHost, unsigned int port)
{
	bool bSuccess = false;
...
bool CTestBase::Test_Sadd()
{
...

	{
		...
		if (m_redis.Sadd("tk_set_8", "value_2") != RC_SUCCESS)
		{
			std::cout << "failed" << std::endl;
			bSuccess = false;
		}
		if (m_redis.Sadd("tk_set_8", "value_10") != RC_SUCCESS)
		{
			std::cout << "failed" << std::endl;
			bSuccess = false;
		}
		if (m_redis.Smembers("tk_set_8", &vecVal) != RC_SUCCESS)
		{
			std::cout << "failed" << std::endl;
			bSuccess = false;
		}

		if (vecVal.size() != 9 || !IsInContainer(vecVal, "value_10"))
		{
			std::cout << "failed" << std::endl;
			bSuccess = false;
		}

		if (m_redis.Sadd("tk_set_10", "value_1") != RC_SUCCESS)
		{
			std::cout << "failed" << std::endl;
			bSuccess = false;
		}

		if (m_redis.Smembers("tk_set_10", &vecVal) != RC_SUCCESS)
		{
			std::cout << "failed" << std::endl;
			bSuccess = false;
		}

		if (vecVal.size() != 1 || !IsInContainer(vecVal, "value_1"))
		{
			std::cout << "failed" << std::endl;
			bSuccess = false;
		}
